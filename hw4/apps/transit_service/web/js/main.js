...
$(document).ready(function () {
  ...
  for (let j = 0; j <= 3; j++) {
    for (let i = 0; i <= 5; i++) {
      if(i !=3){
        beaconparams.position[0] = i * 510 - 1200;
        beaconparams.position[2] = j * 450 - 600;
        beaconparams.name = "beacon" + j + i;
        console.log("creating beacon")
        api.sendCommand("CreateEntity", JSON.parse(JSON.stringify(beaconparams)));
      }
    }
  }
  let droneparams = {
  ...
  for (let j = 0; j < 48; j++) {
    droneparams.name = "drone" + j
    console.log("creating drone")
    api.sendCommand("CreateEntity", JSON.parse(JSON.stringify(droneparams)));
  }
});
...
const onLoad = (gltf, position, scale, start, duration, details, id) => {
  ...
  group.traverse((o) => {
    if (o.isMesh) {
      ...
      if (details.type == "radius_marker") {
        o.material.color.set(new THREE.Color(details.color));
      }
    }
  });
